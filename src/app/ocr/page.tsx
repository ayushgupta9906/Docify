...
export default function OCRPDFPage() {
    ...
    const handleProcess = async () => {
        ...
        try {
            ...
            const job = await api.processTool('ocr', fileIds);
        ...
        <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white dark:from-gray-900 dark:to-gray-800 py-12 px-4">
            <div className="max-w-4xl mx-auto">
                <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
                    ...
                    <p className="text-gray-600 dark:text-gray-400 text-lg">Convert scanned PDF documents into searchable text</p>
                </motion.div>
                    ...
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
                        ...
                        {files.length === 1 && (
                            <div className="mt-8 text-center">
                                <button onClick={handleProcess} className="btn btn-primary text-lg px-12 py-4">Start OCR</button>
                            </div>
                        )}
                    </motion.div>
                )}
                ...
                {processing && (
                    <div className="mt-8">
                        <ProgressIndicator status={status} progress={progress} message={status === 'processing' ? 'Extracting text...' : status === 'completed' ? 'Success!' : 'Error'} />
                        {status === 'completed' && <div className="mt-6 text-center"><DownloadButton onClick={handleDownload} filename="ocr_processed.pdf" /></div>}
